...
class FolderStore {
  ...
  private async save(): Promise<void> {
    await writeFile(await this.file(), JSON.stringify({ folders: this.cache ?? [] }, null, 2), 'utf8')
  }
...
class BookmarkStore {
  ...
  private async save(): Promise<void> {
    await writeFile(await this.file(), JSON.stringify(this.cache ?? {}, null, 2), 'utf8')
  }
  ...
  async setNote(sessionId: string, note: string): Promise<BookmarkEntry | null> {
    ...
    if (!existing.bookmarked && existing.note === '') delete entries[sessionId]
    await this.save()
    return existing
  }
}
 ...
 */
async function snapshotSession(ctx: HostContext, config: ArchiveViewerConfig, sessionId: SessionId, force: boolean): Promise<ArchiveSnapshot | null> {
  ...
  const summary = await generateSummary(ctx, config, sessionId, transcript)
  const snapshot: ArchiveSnapshot = {
    sessionId,
    title: summary?.goal !== '' ? summary?.goal ?? title : title,
...
export function apply(ctx: HostContext, config: ArchiveViewerConfig): void {
  ...
  const dispose = ctx.connection.rpc.handle('/rpc', async (endpoint, payload) => {
    switch (endpoint) {
      ...
      case 'archive.summary.refresh': {
        const sessionId = (payload as { sessionId?: unknown } | null)?.sessionId
        if (typeof sessionId !== 'string' || sessionId === '') return fail('sessionId is required', 'bad-request')
        const snapshot = await snapshotSession(ctx, cfg, sessionId as SessionId, true)
        return snapshot === null
          ? fail('session has no visible conversation', 'empty')
          : ok(snapshot)
      }
